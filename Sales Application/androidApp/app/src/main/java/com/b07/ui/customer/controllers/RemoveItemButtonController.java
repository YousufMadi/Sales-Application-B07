...
public class RemoveItemButtonController implements View.OnClickListener {
   ...
   */
  public RemoveItemButtonController(Context context, EditText itemQuantity, Customer customer,
  ...
  public void onClick(View v) {
    ...
    try {
      ...
      int cartQuantity = cart.getCart().get(item);
      if (quantity > cartQuantity) {
        message = Toast.makeText(appContext, "Don't have that quantity of " + itemName
            + " in the cart", Toast.LENGTH_SHORT);
        message.show();
      } else {
        cart.removeItem(item, quantity);
        MainCustomerView.printCart(cart, inventory);
        message = Toast.makeText(appContext, "Removed " + quantity + " of " + itemName,
            Toast.LENGTH_SHORT);
        message.show();
      }
      ...
      message = Toast.makeText(appContext, "Item does not Exist in Cart!",
          Toast.LENGTH_SHORT);
      message.show();
    }
  }
